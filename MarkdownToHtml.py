...
class MarkdownTiHtml:

    headTag='<head><meta charset="utf-8" /></head>'

    def _init_(self,cssFilePath=none):
        if cssFilePath!=None:
    ...
    def markdownToHtml(self,sourceFilePath,destinationDirectory=None,outputFileName=None):
        ...
        with open(sourceFilePath,'r',encoding='utf8') as f:
            markdownText=f.read()
        #编译出原始HTML文本
        rawHtml = self.headTag+markdown.markdown(markdownText,outpt_format='htmls')
        #格式化HTML文本为可读性更强的格式
        beautifyHtml = BeautifulSoup(rawHtml,'htmlslib').prettify()
        with open(destinationDirectory+outputFileName,'w',endcoding='utf8') as f:
...
if _name_ =="_main_":
    ...
    argv = sys.argv[1:]
    #目前列表argv 可能包含源文件路径之外的元素（即选项信息）
    #程序最后遍历列表argv进行编译markdown时，列表中的元素必须全部是源文件路径
